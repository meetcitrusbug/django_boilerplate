...
class MyStripe():
    ...
    def __init__(self):
        stripe.api_key = 'sk_test_51IqB7USAWJc5b1avefH83xXQ5fPhy36hAjTS8hKl8RR2YYTkPuYap4CJoKs6cuED6HjcA8g5jMhbNZJoWoyUMYvm00QwKWrqUm'
    ...
    def createCustomer(self,user):
        ...
        return stripe.Customer.create(email=user.email,name=user.username)
    ...
    def createPlan(self, amount, interval, product_id, interval_count = 1):
        return  stripe.Plan.create(amount=amount, currency=settings.CURRENCY, interval=interval, product=product_id, interval_count=interval_count)
    ...
    def createCardToken(self, number, exp_month, exp_year, cvc):
