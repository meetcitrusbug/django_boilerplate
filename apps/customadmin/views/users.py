# -*- coding: utf-8 -*-
from apps.customadmin.stripe import MyStripe
...
class UserCreateView(MyCreateView):
    ...
    def get_form_kwargs(self):
        ...
        self.email = kwargs["data"]["email"] if 'data' in kwargs.keys() else ''
        return kwargs
    def get_success_url(self):
        stripe = MyStripe()
        user=User.objects.get(email=self.email)
        UserProfile.objects.create(user=user)
        customer_id = stripe.createCustomer(user)
        user.credentials = customer_id["id"]
        user.save()
        # card_token = stripe.createCardToken('4242424242424242',5,2022,'314')
        # data={'source':card_token["id"]}
        # card_id = stripe.createCard(user.credentials,data)
        # Card.objects.create(user=user,stripe_card_id=card_id["id"],last4=card_id["last4"],card_expiration_date=str(card_id['exp_month'])+'/'+str(card_id['exp_year']))
