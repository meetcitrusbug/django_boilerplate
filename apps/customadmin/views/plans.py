...
from customadmin.forms import PlanChangeForm, PlanCreationForm, PlanFeatureCreationForm, PlanFeatureChangeForm
...
class PlanDetailView(MyDetailView):
    ...
    def get(self, request, pk):
        ...
        self.context['plan_feature_list'] = PlanFeature.objects.filter(plan=self.context['plan_detail'].pk)
...
class PlanFeatureInline(InlineFormSetFactory):
    """Inline view to show Cover within the Parent View"""

    model = PlanFeature
    form_class = PlanFeatureCreationForm
...
class PlanFeatureUpdateInline(InlineFormSetFactory):
    """View to update Cover which is a inline view"""

    model = PlanFeature
    form_class = PlanFeatureChangeForm
    factory_kwargs = {'extra': 1, 'max_num': None, 'can_order': False, 'can_delete': True}
